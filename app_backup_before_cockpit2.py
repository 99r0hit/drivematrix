...
def cockpit1_control_worker():
    """
    Cockpit 1 control path:
        G29 -> per-cockpit sensitivity -> RadioManager -> selected vehicle

    This worker owns only the wheel input device. RadioManager remains the
    owner of the Nano serial connection.
    """
    cockpit_id = 1
    device = None
    radio_id = None

    print("[Control] Cockpit 1 control worker starting")
    ...
    try:
        ...
        if cockpit is None:
            print("[Control] Cockpit 1 does not exist")
            return

        if cockpit.wheel is None:
            print("[Control] Cockpit 1 has no wheel assigned")
            return

        if cockpit.radio_id is None:
            print("[Control] Cockpit 1 has no radio assigned")
            return
        ...
        while not cockpit_control_stop.is_set():
            ...
            if not ok:
                print(
                    f"[Control] Cockpit 1 control send failed "
        ...
        print(f"[Control] Cockpit 1 worker error: {exc}")
        ...
        print("[Control] Cockpit 1 control worker stopped")


def start_cockpit_control_worker():
    global cockpit_control_thread

    if cockpit_control_thread is not None and cockpit_control_thread.is_alive():
        return

    cockpit_control_stop.clear()
    cockpit_control_thread = threading.Thread(
        target=cockpit1_control_worker,
        name="cockpit1-control",
        daemon=True
    )
    cockpit_control_thread.start()


def stop_cockpit_control_worker():
    cockpit_control_stop.set()

    if cockpit_control_thread is not None:
        cockpit_control_thread.join(timeout=2.0)
...
if __name__ == '__main__':
    ...
    # Multi-cockpit control: Cockpit 1 only for the first integrated test.
    # This uses the already-connected RadioManager; it does NOT open Nano
    # serial ports directly and does NOT start the legacy drive worker.
    start_cockpit_control_worker()

    try:
        app.run(host='0.0.0.0', port=5000)
    finally:
        stop_cockpit_control_worker()
