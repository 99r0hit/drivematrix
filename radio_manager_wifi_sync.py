...
class RadioManager:
    ...
    def wifi_sync_only(self):
        """
        Discover Nanos, connect them, discover RXs, dynamically assign
        vehicles, provision Pi Wi-Fi credentials, wait for Nano responses,
        then cleanly disconnect.

        This is intended to be called by the Flask Web UI.
        It deliberately does NOT run the control/ACK test.
        """

        print()
        print("==============================")
        print("DriveMatrix WiFi Sync")
        print("==============================")
        ...
        print()
        print("NANO DISCOVERY")

        radios = self.discover()

        if not radios:
            print("RESULT: NO_RADIOS")
            return False
        ...
        if connected != len(self.radios):
            print("RESULT: CONNECT_INCOMPLETE")
            self.disconnect_all()
            return False
        ...
        if not receivers:
            print("RESULT: NO_RECEIVERS")
            self.disconnect_all()
            return False

        # ----------------------------------------------------
        # 4. Dynamic vehicle assignment
        # ----------------------------------------------------

        print()
        print("DYNAMIC VEHICLE ASSIGNMENT")
        ...
        radio_items = list(self.radios.items())
        ...
        for i in range(assignment_count):
            ...
            radio_id, controller = radio_items[i]
        ...
        try:
            ...
            print(
                f"RESULT: "
                f"WIFI_CREDENTIAL_READ_FAILED: {exc}"
            )
        ...
        # ----------------------------------------------------
        # 6. Provision every assigned radio
        # ----------------------------------------------------
        ...
        # ----------------------------------------------------
        # 7. Read Nano responses
        # ----------------------------------------------------

        print()
        print("NANO WIFI RESPONSES")
        ...
        print(
            "RESULT: "
            "WIFI_PROVISIONING_COMMANDS_SENT"
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        if "--wifi-sync" in sys.argv:

            success = manager.wifi_sync_only()
